...
/**
 * Removes one city from database. The target city must be sent as first element of varargs in {@code execute()} method. Other entities in {@code execute()} method will be ignored.
 */
public final class CityDeleteAsyncTask extends AsyncTask<City, Void, Integer> {
    ...
    protected Integer doInBackground(City... cities) throws IllegalArgumentException {
        ...
        int result = dbContext.cityDao().delete(cities[0]);
